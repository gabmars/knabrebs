...
def ecom_logger(tid, message):
    ...
    with open(os.path.abspath(ecom_dir+'ecom_log'), 'a') as file:
        file.write('{} {} {}\n'.format(str(datetime.datetime.now()), tid, message))
        file.close()
...
def scan(inpt, data, ecom_dir):
    ...
    #    start_time=datetime.datetime.now()
        #</time_monitor>
        sweb=inp[0]
        sys.stdout=open(os.path.abspath(ecom_dir+'/ecomlog/{}_{}.counter_log'.format(str(len(data)), tid)),'w')
        print(len(data))
        title=''
        meta_descr=''
        meta_kw=''
...
#                                row.append(';'.join(list(phones.keys())))
                                row.append(';'.join([re.sub('\D','',pm.raw_string) for pm in phonenumbers.PhoneNumberMatcher(text,'RU')]))
                            except:
                                row.append('')
                            try:
...
if __name__ == '__main__':
    ...
    output_filename=os.path.abspath(ecom_dir+'OUT/ecom_res_'+tid)
    
    ecom_logger(tid,'START')

    try:
        webs=pd.read_csv(sys.argv[1],encoding='1251', sep=';')
    except Exception as e:
    ...
    prcs=[]
    for chunk in chunks:
        p=mp.Process(target=scan, args=(chunk,shared_list,ecom_dir))
        prcs.append(p)
        p.start()
...
#%%    
    ...
    for f in [f for f in os.listdir(ecom_dir+'ecomlog') if tid in f]:
        os.remove(os.path.abspath(ecom_dir+'ecomlog/'+f))

    ecom_logger(tid,'DATA COLLECTED. ELAPSED TIME {}'.format(str(datetime.datetime.now()-start_time)))
    
    writer = pd.ExcelWriter(output_filename+'_temp.xlsx',options={'strings_to_urls': False})
    data.to_excel(writer,index=None)
    writer.close()
    data.columns=['In_Web','In_RegDate','MainContent','Out_Web','<Title>','<Description>','<Keywords>','LinkType','Link','VK','OK','Facebook','Twitter','Instagram','YouTube','Phones','PhonesLabels','INN','KPP','OGRN','BIK','NAME','CS','RS','Email','DomainRegDate','DomainExpiryDate','WHOIS','WHOIS_PHONE','WHOIS_EMAIL','Payment','Logistic','INNS','OGRNS']
    data=data[['In_Web','Out_Web', 'MainContent', '<Title>','<Description>','<Keywords>','LinkType','Link','NAME','INN','KPP','OGRN','BIK','CS','RS','Phones','PhonesLabels','Email','VK','OK','Facebook','Twitter','Instagram','YouTube','In_RegDate','DomainRegDate','DomainExpiryDate','Payment','Logistic','WHOIS','WHOIS_PHONE','WHOIS_EMAIL', 'INNS','OGRNS']]
    data=data.fillna('')
    ...
    for n,i in enumerate(data['In_Web'].drop_duplicates().values.ravel()):
        ...
        for col in ['INN','KPP','OGRN','BIK','CS','RS','Email','VK','OK','Facebook','Twitter','Instagram','YouTube','Payment', 'Logistic']:
    ...
    res=res[['In_Web','Out_Web', 'MainContent','<Title>','<Description>','<Keywords>','Links','NAME','INN','KPP','OGRN','BIK','CS','RS','Phones','PhonesLabels','Email','VK','OK','Facebook','Twitter','Instagram','YouTube','In_RegDate','DomainRegDate','DomainExpiryDate','Payment','Logistic','INNS','OGRNS','WHOIS','WHOIS_PHONE','WHOIS_EMAIL']]
    ...
    try:
        send_mail('studtosber@gmail.com', 'ECOM', 'Result', [output_filename+'.xlsx'])
    except Exception as e:
