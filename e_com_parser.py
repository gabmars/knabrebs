...
#%%
d = {}
words=['Контакты', 'О нас', 'Контактная информация','Реквизиты', 'Контакты и реквизиты','Оплата','Доставка и оплата','Оплата и доставка']
...
for i,n in enumerate(webs.index):
    ...
    try:
            ...
            try:
                ...
                for rs in ['Расчетный счет','Рассч/С','Р/Счет','Р/сч','Р/С']:
                    try:
                        ...
                        pass
                if len([ss for ss in re.findall('\d+',texts[0]) if len(ss)==20])>0:
                    check.append(webs['Web'][n])
                try:
        ...
        print(str(e))
data.columns=['Web','<Title>','<Description>','<Keywords>','LinkType','Link','Phones','INN','KPP','OGRN','BIK','CS','RS','Email','Payment']
data=data[['Web','<Title>','<Description>','<Keywords>','LinkType','Link','INN','KPP','OGRN','BIK','CS','RS','Phones','Email','Payment']]
data=data.fillna('')
...
for n,i in enumerate(data['Web'].drop_duplicates().values.ravel()):
    ...
    for lt,ll in zip(sdf['LinkType'].values.ravel(),sdf['Link'].values.ravel()):
        lll[lt]=ll
    for col in ['Web','<Title>','<Description>','<Keywords>','LinkType']:
        res=res.set_value(n,col,sdf[col].values.ravel()[0])
    res['Link']=str(lll)
    for col in ['INN','KPP','OGRN','BIK','CS','RS','Phones','Email','Payment']:
        res=res.set_value(n,col,sdf.loc[sdf[col].apply(len)==sdf[col].apply(len).max()][col].values.ravel()[0])        
#data.to_excel('ecom_result.xlsx',index=None)
